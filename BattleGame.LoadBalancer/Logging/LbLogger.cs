...
namespace BattleGame.LoadBalancer.Logging;

public static class LbLogger
{
    ...
    private static readonly string ServiceName = FirstNonEmpty(
    ...
    public static void Event(string category, string action, params (string Key, object? Value)[] fields)
    {
        string suffix = fields.Length == 0
            ? string.Empty
            : " " + string.Join(" ", Array.ConvertAll(fields, field => $"{field.Key}={FormatValue(field.Value)}"));
